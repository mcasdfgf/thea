...
class GraphService:
    ...
    def _get_node_visuals(self, node_data: Dict[str, Any]) -> Dict[str, Any]:
        """Determines the color, shape, and size for a node based on its type and state."""
        node_type = node_data.get("type", "DEFAULT")
        node_state = node_data.get("state", "ACTIVE")

        color = COLOR_PALETTE.get(node_type, COLOR_PALETTE["DEFAULT"])
        # Override color if the node is in a special state.
        if node_state == "ARCHIVED":
            color = COLOR_PALETTE["ARCHIVED"]
